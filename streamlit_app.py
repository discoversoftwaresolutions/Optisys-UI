...
def render(client_id):
    ...
    with tab1:
        ...
        customer_id = st.text_input("Client ID", value=client_id)
        if st.button("Stream Logs"):
            sid = f"{product}_{customer_id}"
            ws_url = f"{WS_HOST}/ws/progress?product={product}&customer_id={customer_id}"
    ...
    with tab3:
        ...
        cid = st.text_input("Client ID", value=client_id)
    ...
    with tab4:
        ...
        upload_client = st.text_input("Client ID for Upload", value=client_id)
    ...
    with tab5:
        ...
        stack_state = st.text_area("Current Stack (JSON)", value='{"data_types":["PHI"], "hipaa_certified": false, "data_encryption_at_rest": false}')
                ...
                if recs:
                    for rec in recs:
                        with st.expander(f"💡 {rec['product']} → {rec['reason']}"):
                            if st.button(f"🧩 Trigger {rec['product']} Integration", key=rec['product']):
                                payload = {
